...
@media only screen and (max-width:768px){#results>div{flex-basis:100%;padding:2px;}}`;document.getElementsByTagName("head")[0].appendChild(g)};window.borrarResultados=function(){let t=qsa("#results>div");for(let e=0;e<t.length;e++){t[e].remove()}let e=qs("#caducahoy");if(e&&e!=null){e.checked=false}let o=qs("#invertir");if(o&&o!=null){o.checked=false}};window.reemplazarDisponibles=function(e){borrarResultados();verArticulos(e)};window.iniciarAutoRefrescar=async function(){qs("#boton-auto-refrescar").classList.add("bloqueado");qs("#boton-auto-capturar").classList.add("bloqueado");window.capturar=false;comprobarReservaCatalogo()};window.iniciarAutoCapturar=async function(){qs("#boton-auto-refrescar").classList.add("bloqueado");qs("#boton-auto-capturar").classList.add("bloqueado");window.capturar=true;comprobarReservaCatalogo()};window.comprobarReservaCatalogo=async function(){let t=webActual;let o=false;let e=qs("#plf").value;if(e!="none"&&e>0){if(o){t+="&"}else{o=true;t+="?"}t+="plf_id="+e}let a=qsa("#category option");a.forEach(e=>{if(e.selected){if(o){t+="&"}else{o=true;t+="?"}t+="category="+e.value;t+="&"+e.value+"=on"}});let r=qs("#available").checked;if(r==true){if(o){t+="&"}else{o=true;t+="?"}t+="available="+r}let l=await fetch(t);let i=await l.text();let n=i.indexOf("antiquity/favorite");let s=i.substring(0,n);comprobarLogin(s);let c=s.includes("Reservar");let d=i.substring(n+19);let u=parseInt(d.substring(0,d.indexOf('" method="POST"')));chks++;if(c&&u>idPrimerArticulo){idPrimerArticulo=u;let e=i.indexOf('<div class="row" id="results">');let t=i.indexOf('<div class="row">',e+30);let o=i.substring(e,t);let a=qs("#invertir");if(a&&a!=null){a.checked=false}qs("#results").outerHTML=o;if(window.capturar==true){let e=qs("#results .thumbnail-container form[action$='"+u+"'] button");e.click()}setTimeout(comprobarReservaCatalogo,timeout)}else{try{tituloCatalogo=qs(".container .row h1");tituloCatalogo.textContent=textoTituloCatalogo+" - "+chks}catch(e){}try{document.title=ttl+" "+chks}catch(e){}setTimeout(comprobarReservaCatalogo,timeout)}};if(window.pulsado!=true){resultadosCatalogo=qs("#results");if(resultadosCatalogo){textoTituloCatalogo=qs(".container .row h1").textContent;if(qs("#caducahoy")==null){agregarBotonera()}reemplazarDisponibles(limiteReducido);window.pulsado=true}}